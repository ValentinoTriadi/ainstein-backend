...
import { conversations, conversationHistory } from '@/db/schema';
...
export const conversationHistorySchema = createSelectSchema(conversationHistory, {
  timestamp: z.union([z.string(), z.date()]),
}).openapi('ConversationHistory');

export const listConversationHistorySchema = z.array(conversationHistorySchema);

export const createConversationHistorySchema = createInsertSchema(conversationHistory).omit({
  id: true,
  timestamp: true,
});
...
export type CreateConversationHistory = z.infer<typeof createConversationHistorySchema>;
